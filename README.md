# Connections Game Clone

A clean, from-scratch implementation of the NYT Connections word puzzle game built with React.

## About

Connections is a word puzzle game where players must find groups of four words that share something in common. The game features:

- 16 words arranged in a 4x4 grid
- 4 categories of words to discover
- 4 difficulty levels (indicated by colors)
- Maximum of 4 mistakes allowed
- Shuffle, Clear, and Submit controls

## Features

- ✨ Clean, modern UI design
- 🎮 Full game mechanics (select, submit, shuffle)
- 🎯 Mistake tracking
- 🎊 Win/lose conditions
- 📱 Responsive design for mobile and desktop
- 🔄 Play again functionality

## How to Play

1. Select 4 words that you think belong to the same category
2. Click "Submit" to check your guess
3. If correct, the category is revealed and removed from the grid
4. If incorrect, you lose one of your 4 chances
5. Find all 4 categories to win!

## Installation

```bash
# Install dependencies
npm install

# Start the development server
npm start
```

The game will open in your browser at `http://localhost:3000`

## Project Structure

```
connections-game-clone/
├── public/
│   └── index.html
├── src/
│   ├── App.js          # Main game component and logic
│   ├── App.css         # Game styling
│   ├── index.js        # React entry point
│   └── index.css       # Global styles
├── package.json
└── README.md
```

## Technologies Used

- React 18
- CSS3
- Create React App

## License

MIT

---

Created with ❤️ as a learning project
